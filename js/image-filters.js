...
const filterHandler = {
  ...
  [filters.RANDOM] : function (data) {
    const randomIndexsList = [];
    const max = Math.min(RANDOM_MAX_POSTS, data.length);
    while (randomIndexsList.length < max) {
      const index = getRandomIndex(0, data.length);
      if (!randomIndexsList.includes(index)){
        randomIndexsList.push(index);
      }
    }
    return randomIndexsList.map((index) => data[index]);
  },
